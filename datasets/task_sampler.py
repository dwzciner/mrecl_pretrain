...
class SVHNSampler:
    ...
    def sample_tasks(self, t, train=False):
        dataset = self.task_sampler.get_task_trainset(t, train)
        train_iterator = DataLoader(dataset, batch_size=1, shuffle=True, num_workers=1)
        return train_iterator
...
class SampleSVHN:
    ...
    def add_complete_iterator(self, tasks):
        dataset = self.get_task_trainset(tasks, True)
        train_iterator = DataLoader(dataset, batch_size=15, shuffle=True, num_workers=1)
        self.complete_iterator = train_iterator

        train_iterator2 = DataLoader(dataset, batch_size=1, shuffle=True, num_workers=1)
        self.another_complete_iterator = train_iterator2
    ...
    def get_task_trainset(self, task, train):
        ...
        indices = np.isin(dataset.labels, task).nonzero()[0]

        dataset.data = dataset.data[indices]
        dataset.labels = np.array(dataset.labels)[indices].tolist()
