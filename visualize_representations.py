...
def main(args):
    ...
    dataset = df.DatasetFactory.get_dataset(args.dataset, train=True,path=args.data_path)
    ...
    with torch.no_grad():
        for img, target in iterator:
            ...
            # print(target)
            rep = maml(img, vars=None, rep=True)[0]
            print(f'rep {rep}')
            print(f'rep.shape {rep.shape}')
            # break
            rep = rep.view((-1, 256,64 )).detach().cpu().numpy()
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--dataset', help='Name of experiment', default="svhn")
    ...
    argparser.add_argument("--data-path", type=str, default="../data/SVHN")
    argparser.add_argument('--color', help='Name of experiment', default="YlGn")
    argparser.add_argument('--d', help='feature dimension', default=64, type=int)
