...
def main():
    ...
    # data_train = df.DatasetFactory.get_dataset("svhn", train=True, path=args['path'])

    data_test = df.DatasetFactory.get_dataset("svhn", train=False, path=args['path'])
    data_train_size = int(len(data_test) * 0.75)
    data_test_size = len(data_test) - data_train_size

    data_train, data_test = torch.utils.data.random_split(dataset=data_test, lengths=[data_train_size,data_test_size])
    data_test = data_test.dataset
    ...
    config = mf.ModelFactory.get_model("na", args['dataset'], output_dimension=args['d'])
    ...
    for total_classes in no_of_classes_schedule:
        ...
        for lr_search_runs in range(0, 5):
            ...
            iterator_sorted = torch.utils.data.DataLoader(
                utils.iterator_sorter_omni(data_train, False, classes=no_of_classes_schedule),
                batch_size=256,
                shuffle=args['iid'], num_workers=2)


            iterator_train = torch.utils.data.DataLoader(data_train, batch_size=256,
        ...
        for current_run in range(0, args['runs']):
            ...
            iterator_test = torch.utils.data.DataLoader(data_test, batch_size=256,
                                                        shuffle=False, num_workers=1)
